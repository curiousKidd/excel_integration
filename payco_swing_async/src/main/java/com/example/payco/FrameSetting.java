...
public class FrameSetting extends JFrame implements ActionListener {
    ...
    private java.util.List<ExcelDTO> getExcelData(java.util.List<PaycoDTO> paycoDTOS) {
        ...
        List<CompletableFuture<ExcelDTO>> futures = new ArrayList<>();

        List<CompletableFuture<ExcelDTO>> asyncDTOS = paycoDTOS.stream()
                .parallel()
                .sorted(compare)
                .filter(f -> set.add(f.getTranNumber()) && f.getTranType().equals("승인"))
                .map(m -> CompletableFuture.supplyAsync(() -> ExcelDTO.builder()
        ...
        asyncDTOS.forEach(completableFuture -> {
            try {
                excelDTOS.add(completableFuture.get());
            } catch (Exception e) {
        ...
        paycoDTOS.stream()
                ...
                .map(m -> {
                            CompletableFuture.runAsync(() -> {
                                if (m.getTicketPaymentAmount() == dto.getTicketPaymentAmount()) {
                                    map.put(m.getName(), m.getTicketPaymentAmount());
                                } else if (m.getTranType().equals("받기")) {
